...
class GFTE_POS_DATASET(Dataset):
    ...
    def check_all(self):
        ...
        for idx in range(len(self.imglist)):
            ...
            structs,chunk = self.reallab(idx)
            check = self.check_chunks(structs,chunk)
            if check == 1:
                vlidlist.append(img)
    ...
    def check_chunks(self,structs,chunks):
        ...
        for st in structs:
            id = st['id']
            if id>=len(chunks):
                logging.error("structs is not match chunks")
                print(" check error....")
                return 0
            chk = chunks[id]
    ...
    def remove_empty_cell(self,structs):
       ...
       for st in structs:
           ...
           if text==""or text=='$\\mathbf{}$':  #空cell   # if text=="" or text=='$\\mathbf{}$': #空cell
               continue
           st["id"] = idx
           news.append(st)
           idx += 1
    ...
    def get(self,idx):
        structs,chunks = self.reallab(idx)
